    ...
    try:
        with open(filename, "r", encoding="utf-8") as f:
            for line_num, line in enumerate(f, start=1):
                ...
                parts = line.split(";")
                if len(parts) != 5:
                    print(f'Пропущена строка {line_num}: неверное количество полей ({len(parts)} вместо 5)')
                    continue
                assign_dt_str, due_dt_str, executor, desc, status = parts
                if not validate_status(status):
                    print(f'Пропущена строка {line_num}: недопустимый статус "{status}"')
                    continue
                try:
                    assign_dt = parse_datetime(assign_dt_str)
                    due_dt = parse_datetime(due_dt_str)
                except ValueError:
                    print(f'Пропущена строка {line_num}: ошибка в формате даты ("{assign_dt_str}" или "{due_dt_str}")')
                    continue
                tasks.append({
                    "assign_datetime": assign_dt,
                    "due_datetime": due_dt,
                    "executor": executor,
                    "description": desc,
                    "status": status
